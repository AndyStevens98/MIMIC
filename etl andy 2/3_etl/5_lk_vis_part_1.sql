...
CREATE TABLE mimiciv_etl.lk_admissions_clean AS
SELECT
...
CREATE TABLE mimiciv_etl.lk_transfers_clean AS
SELECT
    src.subject_id                                  AS subject_id,
...
LEFT JOIN
    mimiciv_etl.lk_admissions_clean vis -- associate transfers with admissions according to
...
CREATE TABLE mimiciv_etl.lk_services_duplicated AS
SELECT
    trace_id, COUNT(*) AS row_count
FROM
    mimiciv_hosp.src_services src
GROUP BY
    src.trace_id
HAVING COUNT(*) > 1
;
...
CREATE TABLE mimiciv_etl.lk_services_clean AS
SELECT
    src.subject_id                                  AS subject_id,
    src.hadm_id                                     AS hadm_id,
