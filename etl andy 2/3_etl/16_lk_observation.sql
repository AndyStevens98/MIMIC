...
CREATE TABLE mimiciv_etl.lk_observation_clean AS
-- rule 1, insurance
SELECT
...
INSERT INTO mimiciv_etl.lk_observation_clean
SELECT
    src.subject_id                  AS subject_id,
    src.hadm_id                     AS hadm_id,
    ...
    src.trace_id                    AS trace_id
FROM
    mimiciv_hosp.src_drgcodes src -- drg
INNER JOIN
...
CREATE TABLE mimiciv_etl.lk_obs_admissions_concept AS
SELECT DISTINCT
...
FROM
    mimiciv_etl.lk_observation_clean src
LEFT JOIN
...
CREATE TABLE mimiciv_etl.lk_observation_mapped AS
SELECT
...
FROM
    mimiciv_etl.lk_observation_clean src
LEFT JOIN
    mimiciv_etl.lk_obs_admissions_concept lc
