...
CREATE TABLE mimiciv_etl.lk_micro_cross_ref AS
SELECT
...
FROM
    mimiciv_hosp.src_microbiologyevents src -- mbe
;
...
CREATE TABLE mimiciv_etl.lk_micro_hadm_id AS
SELECT
...
FROM
    mimiciv_etl.lk_micro_cross_ref src
INNER JOIN
...
CREATE TABLE mimiciv_etl.lk_meas_organism_clean AS
SELECT DISTINCT
...
FROM
    mimiciv_hosp.src_microbiologyevents src -- mbe
INNER JOIN
    mimiciv_etl.lk_micro_cross_ref cr
...
CREATE TABLE mimiciv_etl.lk_specimen_clean AS
SELECT DISTINCT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
...
FROM
    mimiciv_etl.lk_meas_organism_clean src -- mbe
INNER JOIN
    mimiciv_etl.lk_micro_cross_ref cr
...
CREATE TABLE mimiciv_etl.lk_meas_ab_clean AS
SELECT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
    cr.start_datetime                           AS start_datetime,
...
CREATE TABLE mimiciv_etl.lk_d_micro_concept AS
SELECT
    dm.itemid                   AS itemid,
...
FROM
    mimiciv_etl.lk_d_micro_clean dm
LEFT JOIN
...
CREATE TABLE mimiciv_etl.lk_specimen_mapped AS
SELECT
    src.subject_id                              AS subject_id,
...
FROM
    mimiciv_etl.lk_specimen_clean src
INNER JOIN
    mimiciv_etl.lk_d_micro_concept mc
        ON src.spec_itemid = mc.itemid
LEFT JOIN
    mimiciv_etl.lk_micro_hadm_id hadm
        ON hadm.event_trace_id = src.trace_id
        AND hadm.row_num = 1
;

ALTER TABLE mimiciv_etl.lk_specimen_mapped add specimen_id serial;
DROP SEQUENCE mimiciv_etl.lk_specimen_mapped_specimen_id_seq CASCADE;

-- -------------------------------------------------------------------
-- lk_meas_organism_mapped
-- -------------------------------------------------------------------

CREATE TABLE mimiciv_etl.lk_meas_organism_mapped AS
SELECT
    src.subject_id                              AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id)         AS hadm_id,
    src.start_datetime::DATE                    AS date_id,
    ...
    src.trace_id                    AS trace_id
FROM
    mimiciv_etl.lk_meas_organism_clean src
INNER JOIN
    mimiciv_etl.lk_d_micro_concept tc
        ON src.test_itemid = tc.itemid
INNER JOIN
    mimiciv_etl.lk_d_micro_concept sc
        ON src.spec_itemid = sc.itemid
LEFT JOIN
    mimiciv_etl.lk_d_micro_concept oc
...
ALTER TABLE mimiciv_etl.lk_meas_organism_mapped add measurement_id serial;

DROP SEQUENCE mimiciv_etl.lk_meas_organism_mapped_measurement_id_seq CASCADE;
...
CREATE TABLE mimiciv_etl.lk_meas_ab_mapped AS
SELECT
    src.subject_id                              AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id)         AS hadm_id,
    src.start_datetime::DATE                    AS date_id,
...
FROM
    mimiciv_etl.lk_meas_ab_clean src
INNER JOIN
    mimiciv_etl.lk_d_micro_concept ac
        ON src.ab_itemid = ac.itemid
LEFT JOIN
    mimiciv_etl.lk_d_micro_concept rc
        ON src.interpretation = rc.source_code
        AND rc.source_vocabulary_id = 'mimiciv_micro_resistance' -- new vocab
LEFT JOIN
    mimiciv_etl.lk_meas_operator_concept opc -- see lk_meas_labevents.sql
...
ALTER TABLE mimiciv_etl.lk_meas_ab_mapped add measurement_id serial;
DROP SEQUENCE mimiciv_etl.lk_meas_ab_mapped_measurement_id_seq CASCADE;

DROP TABLE IF EXISTS mimiciv_etl.lk_micro_cross_ref;
DROP TABLE IF EXISTS mimiciv_etl.lk_micro_hadm_id;
DROP TABLE IF EXISTS mimiciv_etl.lk_meas_organism_clean;
DROP TABLE IF EXISTS mimiciv_etl.lk_specimen_clean;
DROP TABLE IF EXISTS mimiciv_etl.lk_meas_ab_clean;