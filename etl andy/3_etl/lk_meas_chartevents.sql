...
CREATE TABLE public.lk_chartevents_clean AS
SELECT
...
FROM
    public.src_chartevents src -- ce
INNER JOIN
    public.src_d_items di
...
CREATE TABLE public.tmp_chartevents_code_dist AS
...
SELECT
    ...
    'mimiciv_meas_chartev'              AS source_vocabulary_id, -- both obs values and conditions
    COUNT(*)                            AS row_count
FROM
    public.lk_chartevents_clean
GROUP BY
...
CREATE TABLE public.lk_chartevents_concept AS
SELECT
    src.itemid                  AS itemid,
...
FROM
    public.tmp_chartevents_code_dist src
LEFT JOIN
...
DROP TABLE IF EXISTS public.tmp_chartevents_code_dist;
...
CREATE TABLE public.lk_chartevents_mapped AS
SELECT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
    src.stay_id                                 AS stay_id,
    ...
    src.load_table_id       AS load_table_id
FROM
    public.lk_chartevents_clean src -- ce
LEFT JOIN
    public.lk_chartevents_concept c_main -- main
        ON c_main.source_code = src.source_code
        AND c_main.source_vocabulary_id = 'mimiciv_meas_chart'
LEFT JOIN
    public.lk_chartevents_concept c_value -- values for main
...
ALTER TABLE lk_chartevents_mapped add measurement_id serial;

DROP SEQUENCE lk_chartevents_mapped_measurement_id_seq CASCADE;
...
CREATE TABLE public.lk_chartevents_condition_mapped AS
SELECT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
    src.stay_id                                 AS stay_id,
...
FROM
    public.lk_chartevents_clean src -- ce
INNER JOIN
    public.lk_chartevents_concept c_main -- condition domain from values, mapped
