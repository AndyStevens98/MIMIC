...
CREATE TABLE public.lk_diagnoses_icd_clean AS
SELECT
...
CREATE TABLE public.lk_diagnoses_icd_mapped AS
SELECT
    src.subject_id                      AS subject_id,
    src.hadm_id                         AS hadm_id,
    ...
    src.load_table_id       AS load_table_id
FROM
    public.lk_diagnoses_icd_clean src
LEFT JOIN
...
DROP TABLE IF EXISTS public.tmp_seq_num_to_concept;
