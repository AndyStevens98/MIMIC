...
CREATE TABLE public.lk_visit_no_hadm_all AS
-- labevents
SELECT
...
FROM
    public.lk_meas_labevents_mapped src
WHERE
    src.hadm_id IS NULL
UNION ALL
-- specimen
...
CREATE TABLE public.lk_visit_no_hadm_dist AS
SELECT
    src.subject_id                                   AS subject_id,
    ...
    CAST(NULL AS varchar)                            AS admission_location, -- to hospital
    CAST(NULL AS varchar)                            AS discharge_location, -- from hospital
    --
    'no_hadm'                       AS unit_id,
    'lk_visit_no_hadm_all'          AS load_table_id
FROM
    public.lk_visit_no_hadm_all src
GROUP BY
    src.subject_id,
    src.date_id
;
...
CREATE TABLE public.lk_visit_clean AS
SELECT
    src.subject_id                                  AS subject_id,
    ...
    CONCAT(
        ...
        CAST(src.hadm_id AS varchar)
    )                                               AS source_value,
    --
    src.unit_id                     AS unit_id,
    src.load_table_id               AS load_table_id
FROM
    public.lk_admissions_clean src -- adm
UNION ALL
SELECT
    src.subject_id                                  AS subject_id,
    ...
    CONCAT(
        CAST(src.subject_id AS varchar), '|',
        CAST(src.date_id AS varchar)
    )                                               AS source_value,
    --
    src.unit_id                     AS unit_id,
    src.load_table_id               AS load_table_id
FROM
    public.lk_visit_no_hadm_dist src -- adm
;

ALTER TABLE lk_visit_clean add visit_occurrence_id serial;
-- -------------------------------------------------------------------
...
CREATE TABLE public.lk_visit_detail_clean AS
SELECT
    src.subject_id                                  AS subject_id,
    src.hadm_id                                     AS hadm_id,
    src.date_id                                     AS date_id,
    ...
    CONCAT(
        ...
        CAST(src.transfer_id AS varchar)
    )                                               AS source_value,
...
-- ER admissions
-- -------------------------------------------------------------------
INSERT INTO public.lk_visit_detail_clean
SELECT
    src.subject_id                                  AS subject_id,
    src.hadm_id                                     AS hadm_id,
    CAST(src.start_datetime AS DATE)                AS date_id,
    ...
    src.load_table_id               AS load_table_id
FROM
    public.lk_admissions_clean src
WHERE
    src.is_er_admission
;
...
SELECT
    ...
    CONCAT(
        CAST(src.subject_id AS varchar), '|',
        CAST(src.hadm_id AS varchar), '|',
        CAST(src.start_datetime AS varchar)
    )                                               AS source_value,
...
CREATE TABLE public.lk_visit_detail_prev_next AS
SELECT
    ...
    src.load_table_id                 AS load_table_id
FROM
    public.lk_visit_detail_clean src
LEFT JOIN
    public.lk_visit_clean vis
...
CREATE TABLE public.lk_visit_concept AS
SELECT
