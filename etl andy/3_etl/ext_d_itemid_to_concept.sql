...
CREATE TABLE public.d_items_to_concept AS
WITH
counts AS
(
...
CREATE TABLE public.d_labitems_to_concept AS
WITH
counts AS
(
    SELECT
    ...
    FROM
        public.lk_meas_labevents_mapped
    GROUP BY itemid
)
SELECT
    -- itemid
...
FROM
    public.lk_meas_d_labitems_concept src
LEFT JOIN
    counts
        USING (itemid)
ORDER BY
...
CREATE TABLE public.d_micro_to_concept AS
WITH
counts AS
(
    SELECT
