...
-- Rule 1
-- -------------------------------------------------------------------
CREATE TABLE public.lk_prescriptions_clean AS
SELECT
    ...
    CAST(src.ndc AS varchar)     AS ndc_source_code, -- ndc was used for automatic/manual mapping,
    'NDC'                       AS ndc_source_vocabulary,
    src.form_val_disp           AS form_val_disp,
    CAST(REGEXP_EXTRACT(src.form_val_disp, '[-]?[\d]+[.]?[\d]*') AS decimal)  AS quantity,
    -- COALESCE(
    ...
    --     || ' ' || COALESCE(src.prod_strength, '')
        ...
        (CASE WHEN src.drug IN ('Bag', 'Vial', 'Syringe', 'Syringe.',
                                'Syringe (Neonatal)', 'Syringe (Chemo)',
                                'Soln', 'Soln.', 'Sodium Chloride 0.9%  Flush')
...
FROM
    public.src_prescriptions src -- pr
LEFT JOIN
    public.src_pharmacy pharm
...
-- -------------------------------------------------------------------
CREATE TABLE public.lk_pr_ndc_concept AS
SELECT DISTINCT
...
FROM
    public.lk_prescriptions_clean src -- pr
LEFT JOIN
...
-- Rule 1
-- -------------------------------------------------------------------
CREATE TABLE public.lk_pr_gcpt_concept AS
SELECT DISTINCT
...
-- Rule 1
-- -------------------------------------------------------------------
CREATE TABLE public.lk_pr_route_concept AS
SELECT DISTINCT
...
CREATE TABLE public.lk_drug_mapped AS
SELECT
    src.hadm_id                                     AS hadm_id,
    src.subject_id                                  AS subject_id,
