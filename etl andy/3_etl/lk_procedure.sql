...
DROP TABLE IF EXISTS public.lk_datetimeevents_concept;
DROP TABLE IF EXISTS public.lk_proc_event_clean;
DROP TABLE IF EXISTS public.lk_datetimeevents_clean;
...
CREATE TABLE public.lk_hcpcsevents_clean AS
 SELECT
...
FROM
    public.src_hcpcsevents src
INNER JOIN
    public.src_admissions adm
...
CREATE TABLE public.lk_procedures_icd_clean AS
SELECT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
    adm.dischtime                               AS start_datetime,
...
CREATE TABLE public.lk_proc_d_items_clean AS
SELECT
    src.subject_id                      AS subject_id,
    src.hadm_id                         AS hadm_id,
    ...
    src.load_table_id                   AS load_table_id
FROM
    public.src_procedureevents src
WHERE
...
-- -------------------------------------------------------------------
INSERT INTO public.lk_proc_d_items_clean
SELECT
    src.subject_id                      AS subject_id,
    src.hadm_id                         AS hadm_id,
    ...
    src.load_table_id                   AS load_table_id
FROM
    public.src_datetimeevents src -- de
INNER JOIN
    public.src_patients pat
...
CREATE TABLE public.lk_hcpcs_concept AS
SELECT
...
CREATE TABLE public.lk_icd_proc_concept AS
SELECT
...
CREATE TABLE public.lk_itemid_concept AS
SELECT
...
CREATE TABLE public.lk_procedure_mapped AS
SELECT
    ...
    src.load_table_id               AS load_table_id
FROM
    public.lk_hcpcsevents_clean src
LEFT JOIN
    public.lk_hcpcs_concept lc
        ON src.hcpcs_cd = lc.source_code
;

-- Rule 2, ICD

INSERT INTO public.lk_procedure_mapped
SELECT
    src.subject_id                          AS subject_id, -- to person
    src.hadm_id                             AS hadm_id, -- to visit
    src.start_datetime                      AS start_datetime,
    ...
    src.load_table_id               AS load_table_id
FROM
    public.lk_procedures_icd_clean src
LEFT JOIN
    public.lk_icd_proc_concept lc
...
SELECT
    ...
    src.load_table_id               AS load_table_id
FROM
    public.lk_proc_d_items_clean src
LEFT JOIN
    public.lk_itemid_concept lc
