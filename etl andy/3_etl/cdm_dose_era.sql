...
CREATE TABLE public.cdm_dose_era
(
...
-- -------------------------------------------------------------------
CREATE TABLE public.tmp_drugIngredientExp AS
SELECT
...
FROM public.cdm_drug_exposure de
INNER JOIN public.voc_drug_strength ds
    ON de.drug_concept_id = ds.drug_concept_id
INNER JOIN public.voc_concept_ancestor ca
    ON  de.drug_concept_id = ca.descendant_concept_id
    AND ds.ingredient_concept_id = ca.ancestor_concept_id
LEFT JOIN public.voc_concept c
...
CREATE TABLE public.tmp_drugWithDose AS
SELECT
...
FROM public.tmp_drugIngredientExp
;
...
CREATE TABLE public.tmp_cteDoseTarget AS
SELECT
...
FROM public.tmp_drugWithDose dwd
WHERE
    dose_value <> -1
;
...
CREATE TABLE public.tmp_cteDoseEndDates_rawdata AS
SELECT
    person_id                                       AS person_id,
...
SELECT
    ...
    unit_concept_id                                             AS unit_concept_id,
    dose_value                                                  AS dose_value,
    DATE_ADD(CAST(drug_exposure_end_date as DATE), INTERVAL '30' DAY)           AS event_date,
    1                                                           AS event_type,
...
CREATE TABLE public.tmp_cteDoseEndDates_e AS
SELECT
    person_id                                                                   AS person_id,
    drug_concept_id                                                             AS drug_concept_id,
    unit_concept_id                                                             AS unit_concept_id,
...
FROM public.tmp_cteDoseEndDates_rawdata
;
...
CREATE TABLE public.tmp_cteDoseEndDates AS
SELECT
    person_id                                       AS person_id,
    drug_concept_id                                 AS drug_concept_id,
    unit_concept_id                                 AS unit_concept_id,
...
FROM public.tmp_cteDoseEndDates_e
WHERE
...
CREATE TABLE public.tmp_cteDoseFinalEnds AS
SELECT
...
FROM public.tmp_cteDoseTarget dt
INNER JOIN public.tmp_cteDoseEndDates e
...
INSERT INTO public.cdm_dose_era
SELECT
    person_id                           AS person_id,
    drug_concept_id                     AS drug_concept_id,
    unit_concept_id                     AS unit_concept_id,
...
FROM public.tmp_cteDoseFinalEnds
GROUP BY
    person_id,
    drug_concept_id,
    unit_concept_id,
...
ALTER TABLE cdm_dose_era add dose_era_id serial;

DROP SEQUENCE cdm_dose_era_dose_era_id_seq CASCADE;

-- -------------------------------------------------------------------
-- Drop Temporary Tables
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS public.tmp_drugIngredientExp;
DROP TABLE IF EXISTS public.tmp_drugWithDose;
DROP TABLE IF EXISTS public.tmp_cteDoseTarget;
DROP TABLE IF EXISTS public.tmp_cteDoseEndDates_rawdata;
DROP TABLE IF EXISTS public.tmp_cteDoseEndDates_e;
DROP TABLE IF EXISTS public.tmp_cteDoseEndDates;
DROP TABLE IF EXISTS public.tmp_cteDoseFinalEnds;
