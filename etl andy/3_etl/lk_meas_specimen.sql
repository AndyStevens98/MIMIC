...
CREATE TABLE public.lk_micro_cross_ref AS
SELECT
    microevent_id                                    AS trace_id_ab, -- for antibiotics
    FIRST_VALUE(src.microevent_id) OVER (
        PARTITION BY
            src.subject_id,
            src.hadm_id,
...
FROM
    public.src_microbiologyevents src -- mbe
;
...
CREATE TABLE public.lk_micro_hadm_id AS
SELECT
...
CREATE TABLE public.lk_meas_organism_clean AS
SELECT DISTINCT
...
FROM
    public.src_microbiologyevents src -- mbe
INNER JOIN
    public.lk_micro_cross_ref cr
        ON src.microevent_id = cr.trace_id_org
;
...
CREATE TABLE public.lk_specimen_clean AS
SELECT DISTINCT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
...
FROM
    public.lk_meas_organism_clean src -- mbe
INNER JOIN
    public.lk_micro_cross_ref cr
...
CREATE TABLE public.lk_meas_ab_clean AS
SELECT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
    cr.start_datetime                           AS start_datetime,
    ...
    src.load_table_id               AS load_table_id,
    0                               AS load_row_id,
    src.microevent_id                    AS trace_id         -- trace_id for antibiotics, no grouping is needed
FROM
    public.src_microbiologyevents src
INNER JOIN
    public.lk_micro_cross_ref cr
        ON src.microevent_id = cr.trace_id_ab
WHERE
...
CREATE TABLE public.lk_d_micro_concept AS
SELECT
    dm.itemid                   AS itemid,
...
CREATE TABLE public.lk_specimen_mapped AS
SELECT
    src.subject_id                              AS subject_id,
    ...
    src.load_table_id               AS load_table_id
FROM
    public.lk_specimen_clean src
INNER JOIN
    public.lk_d_micro_concept mc
        ON src.spec_itemid = mc.itemid
LEFT JOIN
    public.lk_micro_hadm_id hadm
        ON hadm.event_trace_id = src.trace_id
        AND hadm.row_num = 1
;

ALTER TABLE lk_specimen_mapped add specimen_id serial;
-- -------------------------------------------------------------------
-- lk_meas_organism_mapped
-- -------------------------------------------------------------------

CREATE TABLE public.lk_meas_organism_mapped AS
SELECT
    src.subject_id                              AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id)         AS hadm_id,
    CAST(src.start_datetime AS DATE)            AS date_id,
    ...
    src.load_table_id               AS load_table_id
FROM
    public.lk_meas_organism_clean src
INNER JOIN
    public.lk_d_micro_concept tc
        ON src.test_itemid = tc.itemid
INNER JOIN
    public.lk_d_micro_concept sc
        ON src.spec_itemid = sc.itemid
LEFT JOIN
    public.lk_d_micro_concept oc
...
ALTER TABLE lk_meas_organism_mapped add measurement_id serial;
-- -------------------------------------------------------------------
-- lk_meas_ab_mapped
--
--      (gcpt) brand new vocab -> mimiciv_micro_resistance
...
CREATE TABLE public.lk_meas_ab_mapped AS
SELECT
    src.subject_id                              AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id)         AS hadm_id,
    CAST(src.start_datetime AS DATE)            AS date_id,
...
ALTER TABLE lk_meas_ab_mapped add measurement_id serial;