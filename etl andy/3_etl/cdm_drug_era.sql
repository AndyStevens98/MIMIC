...
-- -------------------------------------------------------------------
CREATE TABLE public.tmp_pretarget_drug
AS SELECT
...
FROM
    public.cdm_drug_exposure d
JOIN
    public.lk_join_voc_drug v
        ON v.descendant_concept_id = d.drug_concept_id
WHERE
    d.drug_concept_id != 0
;

CREATE TABLE public.tmp_subenddates_un_drug
    AS SELECT
...
CREATE TABLE public.tmp_subenddates_rows_drug
AS SELECT
    person_id                       AS person_id,
    ingredient_concept_id           AS ingredient_concept_id,
...
FROM
    public.tmp_subenddates_un_drug
;

CREATE TABLE public.tmp_subenddates_drug
AS SELECT
    person_id               AS person_id,
    ingredient_concept_id   AS ingredient_concept_id,
    event_date              AS end_date
FROM
    public.tmp_subenddates_rows_drug e
WHERE
    (2 * e.start_ordinal) - e.overall_ord = 0
;

CREATE TABLE public.temp_ends_drug
AS SELECT
...
FROM
    public.tmp_pretarget_drug dt
JOIN
    public.tmp_subenddates_drug e
...
CREATE TABLE public.tmp_sub_drug
AS SELECT
    ROW_NUMBER() OVER (
        PARTITION BY
            person_id,
...
CREATE TABLE public.tmp_finaltarget_drug
AS SELECT
...
CREATE TABLE public.tmp_enddates_un_drug
    AS SELECT
        person_id                                       AS person_id,
        ingredient_concept_id                           AS ingredient_concept_id,
...
CREATE TABLE public.tmp_enddates_rows_drug
AS SELECT
    person_id                       AS person_id,
    ingredient_concept_id           AS ingredient_concept_id,
    event_date                      AS event_date,
    ...
    ROW_NUMBER() OVER (
      ...
      -- this re-numbers the inner UNION so all rows are numbered ordered by the event date
FROM
    public.tmp_enddates_un_drug
;

CREATE TABLE public.tmp_enddates_drug
AS SELECT
    person_id                                       AS person_id,
    ingredient_concept_id                           AS ingredient_concept_id,
    DATE_SUB (CAST(event_date as DATE), INTERVAL '30' DAY)          AS end_date  -- unpad the end date
FROM
    public.tmp_enddates_rows_drug e
WHERE
    (2 * e.start_ordinal) - e.overall_ord = 0
;

CREATE TABLE public.tmp_drugera_ends_drug
AS SELECT
...
FROM
    public.tmp_finaltarget_drug ft
JOIN
    public.tmp_enddates_drug e
...
CREATE TABLE public.cdm_drug_era
(
...
-- -------------------------------------------------------------------
INSERT INTO public.cdm_drug_era
SELECT
    person_id                                                           AS person_id,
    ...
    EXTRACT(DAY from drug_era_end_date -
              MIN(drug_sub_exposure_start_date) - interval '1 day' * SUM(days_exposed))    AS gap_days,
-- --
    'drug_era.drug_exposure'                                            AS unit_id,
    CAST(NULL AS varchar)                                                AS load_table_id
FROM
    public.tmp_drugera_ends_drug
GROUP BY
    person_id,
    drug_era_end_date,
    ingredient_concept_id
...
ALTER TABLE cdm_drug_era add drug_era_id serial;

DROP SEQUENCE cdm_drug_era_drug_era_id_seq CASCADE;

-- -------------------------------------------------------------------
-- Drop temporary table
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS public.tmp_drugera_ends_drug;
DROP TABLE IF EXISTS public.tmp_enddates_drug;
DROP TABLE IF EXISTS public.tmp_finaltarget_drug;
DROP TABLE IF EXISTS public.tmp_enddates_un_drug;
DROP TABLE IF EXISTS public.tmp_sub_drug;
DROP TABLE IF EXISTS public.temp_ends_drug;
DROP TABLE IF EXISTS public.tmp_pretarget_drug;
DROP TABLE IF EXISTS public.tmp_subenddates_un_drug;
DROP TABLE IF EXISTS public.tmp_subenddates_rows_drug;
DROP TABLE IF EXISTS public.tmp_subenddates_drug;
DROP TABLE IF EXISTS public.tmp_enddates_rows_drug;
-- -------------------------------------------------------------------
-- Loading finished
-- -------------------------------------------------------------------