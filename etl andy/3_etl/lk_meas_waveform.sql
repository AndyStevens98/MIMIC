...
DROP TABLE IF EXISTS public.lk_wf_clean;

CREATE TABLE public.lk_waveform_clean AS
SELECT
    ...
    CASE
        WHEN EXTRACT(YEAR FROM src.mx_datetime) < pat.anchor_year
            THEN DATETIME(pat.anchor_year, EXTRACT(MONTH FROM src.mx_datetime), EXTRACT(DAY FROM src.mx_datetime),
...
FROM
    public.src_waveform_mx src -- wm
INNER JOIN
    public.src_waveform_header wh
...
INSERT INTO public.lk_waveform_clean
SELECT
    wh.subject_id                           AS subject_id,
    CONCAT(
        wh.reference_id, '.',
    ...
    src.load_table_id                       AS load_table_id
FROM
    public.src_waveform_mx_3 src -- wm
INNER JOIN
    public.src_waveform_header_3 wh
...
CREATE TABLE public.lk_wf_hadm_id AS
SELECT
...
CREATE TABLE public.lk_meas_waveform_mapped AS
SELECT
...
LEFT JOIN
    public.voc_concept_relationship vr
...
LEFT JOIN
    public.lk_wf_hadm_id hadm
        ON hadm.event_trace_id = src.trace_id
        AND hadm.row_num = 1
;

ALTER TABLE lk_meas_waveform_mapped add measurement_id serial;

DROP SEQUENCE lk_meas_waveform_mapped_measurement_id_seq CASCADE;